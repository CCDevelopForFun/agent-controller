 ...
 *
 * - `tracing` (slice 5.1): on the host, gates OTel SDK init in agentctl.
 *   Slice 5.3 wires the SAME flag into the Pi adapter: when true and
 *   OTEL_EXPORTER_OTLP_ENDPOINT is set, the adapter initializes its own
 *   OTel SDK and nests its spans under the TRACEPARENT delivered by
 *   slice 5.2's KubernetesBackend / LocalBackend env injection.
 *
 * - `captureContent` (slice 5.3): when true, the adapter attaches the
 *   actual prompt messages, completion text, tool-call arguments, and
 *   tool-call results as span attributes (`gen_ai.prompt`,
 *   `gen_ai.completion`, `gen_ai.tool.call.args`, `gen_ai.tool.call.result`).
 *   Off by default for privacy — only enable when shipping to a backend
 *   approved to store conversation content. Slice 5.6 will gate this
 *   behind workspace policy.
 */
