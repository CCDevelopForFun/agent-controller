...
| `Tool` (registry manifest) | ✅ | ⚠️ — only Pi *built-in* tools (`bash`, `read`, `edit`, `write`) map onto opencode permissions; custom Pi-extension tools (entrypoint set, builtin unset) are rejected at **compile time** in `agentctl compile` as of v0.3.4 (adapter startup retains the same check as defense-in-depth) | ⚠️ — Pi built-in tools (`bash`, `read`, `edit`, `write`) are recognized in the spec but the codex sandbox provides its own equivalent tool surface; custom Pi-extension tools (entrypoint set, `builtin` unset) are **rejected at adapter startup** with a field-naming error | ⚠️ — Pi built-ins map onto SDK tool names (bash→Bash, read→Read, edit→Edit, write→Write); custom Pi-extension tools rejected at **compile time** | 🚧 |
...
| Server-name validation | (no opencode equivalent) | ⚠️ — opencode-only: server names must match `[A-Za-z0-9._-]+`. Names that would overmatch built-in permissions via the wildcard `<name>_*` allow grant (e.g. `repo` matches `repo_clone`) or that sanitize to the same key as another declared server (e.g. `github.com` and `github_com`) are rejected at adapter startup | ⚠️ — codex-only: server names must match `[A-Za-z0-9_-]+` (stricter — no dots). Rejected at adapter startup. | (no claude equivalent) — `toMcpServerConfig()` performs no name validation; any string is accepted | 🚧 |
...
| Session resume (`--resume`) | ✅ | ✅ — since slice 8.1 (stable per-session data dir + reuse-by-id) | ✅ — stable per-session `CODEX_HOME` + persisted `thread_id` file; `codex exec --thread-id` resumes the conversation | ✅ — mapped to the SDK's `resume` option; requires the SDK's own session id captured from a prior run's `system` init message | 🚧 |
| Interactive REPL (`agentctl chat`) | ❌ — planned v0.1.11+ | ❌ — same status as Pi; the CLI-level flow hasn't been built yet | ❌ — same status as Pi and opencode | ❌ — same status as Pi | 🚧 |
| Long-lived daemon (`agentctl serve`) | ✅ — v0.8; HTTP/SSE via `agentctl serve` | ✅ — v0.8; HTTP/SSE via `agentctl serve` | ✅ — v0.8; `agentctl serve` uses the same adapter-dispatch path; works with the codex adapter | ✅ — v0.8; `agentctl serve` uses the same adapter-dispatch path; works with the claude adapter | 🚧 |
